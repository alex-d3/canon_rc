#include "msprf24.h"
#include "nrf24l01_userconfig.h"
#include "msp430g2231_spi.h"

/* CE (Chip Enable/RF transceiver activate signal) and CSN (SPI chip-select) operations. */
#define CSN_LOW nrfCSNportout &= ~nrfCSNpin
#define CSN_HIGH nrfCSNportout |= nrfCSNpin
#define CE_HIGH nrfCEportout |= nrfCEpin
#define CE_LOW nrfCEportout &= ~nrfCEpin
...
void msprf24_standby()
{
	...
	if (state == RF24_STATE_POWERDOWN) {  // If we're powering up from deep powerdown...
		//CE_HIGH;  // This is a workaround for SI24R1 chips, though it seems to screw things up so disabled for now til I can obtain an SI24R1 for testing.
